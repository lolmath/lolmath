...
const comparedPlayers = topSoloDuoPlayers.slice(0, 4);

/**
 * `orientation="vertical"` is the transposed table, the one that compares a
 * handful of records field by field: a row per field, a column per record, and
 * the header reading down the leading column rather than across the top.
 *
 * React Aria's table is row major whichever way it is read, so the flip is in
 * the data — the fields are the `TableBody` items and the records the columns —
 * and in the CSS, which draws the rule down the table instead of under the
 * header. The column carrying the field names is the one marked `isRowHeader`,
 * which is what makes its cells headers to a screen reader too.
 */
export const Vertical: Story = {
	render: (args) => (
		<Table {...args} aria-label="Players compared" orientation="vertical">
			<TableHeader>
				<TableColumn id="field" isRowHeader>
					Stat
				</TableColumn>
				<Collection items={comparedPlayers}>
					{(player) => <TableColumn align="end">{player.summoner}</TableColumn>}
				</Collection>
			</TableHeader>
...
/**
 * A vertical table of a single record has nothing to call its one column. The
 * header row cannot be left out — React Aria builds the table's columns from
 * it, and they are what names every cell to a screen reader — so
 * `isVisuallyHidden` clips it out of the layout instead, leaving the stat card
 * that is a table only underneath.
 */
export const VerticalWithoutHeaderRow: Story = {
	render: (args) => (
		<Table {...args} aria-label="Faker" orientation="vertical">
			...
			<TableBody items={playerFields}>
				{(field) => (
					<TableRow>
						<TableCell>{field.name}</TableCell>
						<TableCell align="end">{topSoloDuoPlayers[0][field.id]}</TableCell>
					</TableRow>
				)}
			</TableBody>
		</Table>
